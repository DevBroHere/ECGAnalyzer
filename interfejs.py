...
def changeAnalyzeSection(arg):
    if arg == "Right":
        ...
        frame_Analysis.grid(row=1, column=1, sticky="nsew")
    ...
    if arg == "Left":
        ...
        frame_Analysis.grid(row=1, column=0, sticky="nsew")
...
sb_textbox = tk.Scrollbar(frame_Analysis, orient='horizontal')
textbox = tk.Text(frame_Analysis, width = 30, height = 40, wrap = "none", xscrollcommand = sb_textbox.set)
#textbox.grid(row = 1, column = 0)
textbox.pack()
#sb_textbox.place(in_ = textbox, relx = 0, rely = 1., relwidth = 1.)
#sb_textbox.grid(row = 2, column = 0, sticky = "ew")
sb_textbox.pack(fill = BOTH)
sb_textbox.config(command = textbox.xview)

button_savetxt = ttk.Button(frame_Analysis, text = "Save",
                              command = lambda: savetxt())
#button_savetxt.grid(row=3, column = 0)
button_savetxt.pack()
